from flask import Flask, jsonify, request # imports flask and allows to instanciate app
from flask_sqlalchemy import SQLAlchemy # this is Object- relational mapping
from sqlalchemy.orm import DeclarativeBase, Mapped, mapped_column, Session #this is a class that all of our classes with inherit
# provides base functionality for converting python objects to rows of data
from sqlalchemy import select, delete# 
from flask_marshmallow import Marshmallow # creates our schema to validate incoming & outgoing data
...
app.config["SQLALCHEMY_DATABASE_URI"] = 'mysql+mysqlconnector://root:Gitkoding2024$@localhost/e_commerce_db2'
#  'mysql+mysqlconnector://root:your_password@localhost/e_commerce_db'
#                           user  password                dac   tabase name
# create a base class for all of our Models (classes that become tables) to inherit from
# the child classes can then breate attributes that become columns inside of tables in db
...
#================================DB Models=================================================
class Customer(Base): # importing the Base class gives this class model funtionality
    __tablename__ = 'Customers' # sets the name of the table in our database
    # type hinting - column name is an attribute and we're creating a type
...
# creating Orders and a one to many relationship between Customer and Order
class Order(Base):
...
with app.app_context():
    ...
    db.create_all() # creates tables if they don't exist, and does nothing if it does exist
...
def get_customers():
    ...
    # uses the python class as a representation for the Customers table
    result = db.session.execute(query).scalars() # returns a lisst of objects (instances of the customer class)
...
def add_cusomer():
    try:
        ...
        return jsonify(err.messages), 400 # Bad Request - insufficiant data or mismatched type
...
def update_customer(id):
    with Session(db.engine) as session:
        with session.begin():
            ...
            # grabbing the 1st result from scalars, returning 
            #the object out of the list of results
            result = session.execute(query).scalar()
            if result is None:
                return jsonify({'message': 'Customer not found'})
            # seeting a variable to the result
            customer = result
            ...
            #update the customers object with the values from the incoming data
...
def get_products_by_name():
    ...
    search = f'%{name}%' # % is like wildcard in sql
...
#create products
#i need a name and price for a product from the user
    ...
    with session(db.engine) as session:
        with session.begin():
           #then we have to find the product in the db
            ...
            #once we have the product and valid date, we can update!
            for field, value in product_data.items():
                setattr(product, field, value)
            # save to db
            session.commit()
            return jsonify({'Message': f'Product for {product_id} updated successfully'}), 200 # update successful
#delete products
...
def delete_product(product_id):
    ...
    with db.session.begin():
        result = db. session.execute(delete_statement)
        if result.rowcount == 0:
        # if product isn't there
            return jsonify({'error': f'Product with id of {product_id} doesnt exist '}), 404
